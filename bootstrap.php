...
if ( ! function_exists( 'dws_wp_framework_output_requirements_error' ) ) {
	 ...
	 */
	function dws_wp_framework_output_requirements_error($component_name, $min_php_version, $min_wp_version, $args = array()) {
		add_action( 'admin_notices', function() use ($component_name, $min_php_version, $min_wp_version, $args) {
			extract($args, EXTR_SKIP); // Make the variables available to the template.
...
if ( ! function_exists( 'dws_wp_framework_output_installation_error' ) ) {
	 ...
	 * @param   array   $args               Associative array of other variables that should be made available in the template's context.
	 */
	function dws_wp_framework_output_installation_error($component_name, $args = array()) {
		add_action( 'admin_notices', function() use ($component_name, $args) {
			extract($args, EXTR_SKIP); // Make the variables available to the template.
...
if ( dws_wp_framework_check_php_wp_requirements_met($min_php_version, $min_wp_version) ) {
	...
	dws_wp_framework_output_requirements_error(DWS_WP_FRAMEWORK_CORE_NAME, $min_php_version, $min_wp_version);
}